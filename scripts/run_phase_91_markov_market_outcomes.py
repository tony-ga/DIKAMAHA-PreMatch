"""Materializa outcomes prospectivos Markov por equipo y mitad.
...
from src.count_market_outcomes import (  # noqa: E402
    EspnMarkovMarketOutcomeParser,
)
...
OUTPUT = ROOT / "artifacts/phase_91_markov_market_outcomes"
RAW_STORE = ROOT / "data/phase_90/raw_responses_v2.sqlite"
COHORT_STORE = ROOT / "data/phase_90/markov_market_cohort_v2.sqlite"
CACHE = ROOT / "data/cache/espn_phase91"
PREDICTIONS = (
    ROOT / "artifacts/phase_90_markov_market_prospective/predictions.json")
...
def _repositories() -> tuple[Any, Any, Any]:
    """Abre stores aislados mediante SQLAlchemy."""
...
def _provider(
    ...
    """Construye transporte raw-first post-match."""
...
def _eligible(
    ...
    """Selecciona fixtures asentables aún sin outcome."""
...
def _competition_id(summary: dict[str, Any]) -> str:
    """Extrae competition ID del summary persistido."""

    competitions = (summary.get("header") or {}).get("competitions")
    competition = (
        competitions[0] if isinstance(competitions, list)
        and competitions else None)
    identifier = (
        competition.get("id") if isinstance(competition, dict) else None)
...
def _capture(
    ...
    """Persiste summary y todas las páginas de plays."""
...
def _materialize(
    rows: list[FrozenCountPrediction],
    raw_repository: SqlAlchemyRawResponseRepository,
    outcome_repository: SqlAlchemyCountOutcomeRepository,
) -> dict[str, Any]:
    """Captura y reconcilia cada outcome elegible."""

    parser, inserted, rejected = EspnMarkovMarketOutcomeParser(), 0, []
    for prediction in rows:
        try:
            ...
            LOGGER.warning(
                "phase91_rejected league=%s match=%s reason=%s",
                prediction.league_slug, prediction.match_id, error)
    return {"eligible": len(rows), "inserted": inserted,
            "rejected": rejected}


def _prediction_hash(rows: list[FrozenCountPrediction]) -> str:
    """Calcula hash canónico de predicciones inmutables."""
...
def _raw_count() -> int:
    """Cuenta payloads raw sin abrir sus cuerpos."""

    engine = create_engine(f"sqlite+pysqlite:///{RAW_STORE}")
    with sessionmaker(bind=engine)() as session:
        return int(session.execute(
            select(func.count(RawResponse.id))).scalar_one())
...
def _report(classification: str, coverage: dict[str, Any]) -> str:
    ...
    return (
        "# Fase 91 — outcomes prospectivos Markov\n\n"
...
def _publish(
    ...
    """Publica cobertura sin ejecutar scoring prematuro."""

    complete = len(outcomes) == len(predictions) and bool(predictions)
    classification = (
        "ready_for_next_phase" if complete else "insufficient_coverage")
    coverage = {
    ...
    _write("config.json", {
        "version": "markov_market_outcomes_v1",
        "settlement_delay_hours": 3})
    _write("input_manifest.json", {
        "phase90_predictions_sha256": _sha(PREDICTIONS)})
...
if __name__ == "__main__":
    ...
    result = run()
    assert result["coverage"]["outcomes"] <= result["coverage"]["predictions"]
    assert result["classification"] in {
        "insufficient_coverage", "ready_for_next_phase"}
    LOGGER.info("Fase 91: %s", result["classification"])
