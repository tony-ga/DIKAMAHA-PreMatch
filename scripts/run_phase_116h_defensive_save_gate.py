"""Gate histórico de la señal defensiva `save` (`DEC-217`, Fase 116H).

Compara el motor con y sin la señal defensiva `save` sobre exactamente los
mismos partidos y snapshots. Fase 116G midió que `save` aporta información
incremental sobre los goles de la ventana siguiente -delta de deviance
`+0.000987`, IC95% `[+0.000679, +0.001278]`, 9,405 partidos fuera de muestra-
con coeficiente negativo: quien acumula paradas está defendiendo. Aquí se
comprueba si eso se traduce en mejores probabilidades servidas.

La consulta de eventos se extiende para incluir `save`, que existe en
`prospective_staging_v2.events` (81,872 filas) y que el query oficial excluye
por su `WHERE event_type IN (...)`.
...
Uso:
    python -m scripts.run_phase_116h_defensive_save_gate
...
EXTENDED_EVENT_QUERY = """
    SELECT
        e.provider_match_id,
        COALESCE(NULLIF(e.provider_event_id, ''), e.event_hash) AS event_id,
        CASE WHEN e.event_type_raw = 'save' THEN 'auxiliary' ELSE e.event_type END AS event_type,
        e.event_type_raw,
    ...
    WHERE (e.event_type IN ('goal','shot_on_target','shot_off_target','shot_blocked',
                            'corner','foul','yellow','red','substitution',
...
def _extended_events(database_url: str) -> dict[int, list[dict[str, Any]]]:
    """Lee eventos incluyendo `save`, en modo estrictamente de lectura."""

    engine = create_engine(database_url, future=True, pool_pre_ping=True)
    events: dict[int, list[dict[str, Any]]] = {}
    with engine.connect() as connection:
        connection.execute(text("SET TRANSACTION READ ONLY"))
        for raw in connection.execute(text(EXTENDED_EVENT_QUERY)).mappings():
            events.setdefault(int(raw["provider_match_id"]), []).append({
                "event_id": str(raw["event_id"]),
                "event_type": str(raw["event_type"]),
                "event_type_raw": raw["event_type_raw"],
...
OUTPUT = ROOT / "artifacts/phase_116h_defensive_save_gate"
...
def _replay(
    ...
    markov_config: MarkovLiveConfig | None = None,
) -> list[dict[str, Any]]:
    """Replay idéntico al oficial, con las configuraciones inyectadas."""

    markov = MarkovLiveV1(markov_config or MarkovLiveConfig())
...
def main() -> int:
    ...
    matches, _official_events, database_audit = read_historical_database(database_url)
    events = _extended_events(database_url)
    saves = sum(
        1 for rows in events.values() for r in rows
        if str(r.get("event_type_raw") or "") == "save")
    print(f"  eventos con `save` incluidos: {saves}", flush=True)
    ...
    variants = {
        "sin_save": (LiveProbabilityEngineConfig(), MarkovLiveConfig()),
        "con_save": (
            LiveProbabilityEngineConfig(),
            MarkovLiveConfig(enable_defensive_save_signal=True)),
    }
    ...
    technical_ok = all(
        ...
        for block in splits.values() for name in variants
    )
